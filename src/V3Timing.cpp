...
class TimingControlVisitor final : public VNVisitor {
    ...
    AstVarScope* getCreateDynamicTriggerScheduler() {
        ...
        // Register for eventsPending (DPI export timing support)
        m_netlistp->dynamicTriggerSchedulerp(m_dynamicSchedp->varp());
    ...
    AstVarScope* getCreateTriggerSchedulerp(AstSenTree* const sentreep) {
        if (!sentreep->user1p()) {
            ...
            // Register for eventsPending (DPI export timing support)
            m_netlistp->addTriggerSchedulerp(trigSchedp->varp());
        }
    ...
    void visit(AstCFunc* nodep) override {
        ...
        if (nodep->dpiExportImpl() && !nodep->dpiExportTiming()) {
            // Direct timing controls were handled by V3Task (dpiExportTiming flag).
            // If we get here with AstCAwait nodes, it means the DPI export calls
            // another suspendable task (indirect timing), which the dispatcher
            // generated by V3Task does not handle. Error in that case.
            nodep->exists([](AstCAwait* const awaitp) -> bool {
                awaitp->v3warn(E_UNSUPPORTED,
                               "Unsupported: Timing controls inside DPI-exported tasks"
                               " (indirect via calling another task with timing)");
                return true;
            });
        }
    }
